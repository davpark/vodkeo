...
async function requireAdmin(req, res) {
    ...
    const user = await db_1.prisma.userProfile.findUnique({ where: { did } });
    if (!user?.isAdmin) {
...
async function recalculateScore(postId) {
    ...
    await db_1.prisma.post.update({ where: { id: postId }, data: { score } });
}
app.post('/api/users', async (req, res) => {
    try {
        ...
        const existing = await db_1.prisma.userProfile.findUnique({ where: { did } });
        if (existing) {
            const profile = await db_1.prisma.userProfile.update({
                where: { did },
                data: { handle },
            });
            res.json(profile);
            ...
            const profile = await db_1.prisma.userProfile.create({
...
app.get('/api/users/posts', async (req, res) => {
    ...
    try {
        const posts = await db_1.prisma.post.findMany({
            where: { authorDid: did, deleted: false },
...
app.delete('/api/users', async (req, res) => {
    ...
    try {
        await db_1.prisma.comment.deleteMany({ where: { authorDid: did } });
        await db_1.prisma.post.deleteMany({ where: { authorDid: did } });
        await db_1.prisma.userProfile.delete({ where: { did: did } });
...
app.get('/api/u/:handle', async (req, res) => {
    try {
        const profile = await db_1.prisma.userProfile.findFirst({
...
app.get('/api/posts/random', async (req, res) => {
    try {
        const count = await db_1.prisma.post.count({ where: { published: true, deleted: false } });
        if (count === 0) {
...
app.post('/api/posts', async (req, res) => {
    try {
        ...
        if (Array.isArray(tags) && tags.includes('news')) {
            const user = await db_1.prisma.userProfile.findUnique({ where: { did: authorDid } });
            if (!user?.isAdmin) {
...
app.post('/api/posts/:id/view', async (req, res) => {
    try {
        ...
        await db_1.prisma.post.update({ where: { id }, data: { views: { increment: 1 } } });
...
app.delete('/api/comments/:id', async (req, res) => {
    try {
        ...
        await db_1.prisma.comment.delete({ where: { id: Number(req.params.id) } });
        res.json({ success: true });
    }
    catch (error) {
